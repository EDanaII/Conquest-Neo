...
public sealed class GenStarsHandler : ICommandHandler {
    ...
    public void Run(string[] args, IGameIO io) {
        int seed = args.TryGet("--seed", 12345);
        int count = args.TryGet("--count", 20);
        int w = args.TryGet("--w", 64);
        int h = args.TryGet("--h", 64);
        int space = args.TryGet("--space", 3);

        var rng = new CRandRng( ); rng.Seed((uint)seed);
